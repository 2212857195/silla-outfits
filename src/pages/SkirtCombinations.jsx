...
const SkirtCombinations = () => {
  const navigate = useNavigate();

  // فلترة المنتجات لتشمل فقط "تركيبات التنانير"
  const filteredProducts = productsData.filter(
    (product) => product.category === "skirt-combinations"
  );

  // العودة للبداية عند فتح الصفحة
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#fff5f8] pb-20" dir="rtl">
      {/* هيدر الصفحة بتنسيق Silla Outfits */}
      <header className="sticky top-0 z-40 bg-[#fff5f8]/80 backdrop-blur-md px-4 py-6 border-b border-[#fce4ec]/50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="w-10"></div> {/* موازن للمسافات */}
        </div>
      </header>
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-8">
            {filteredProducts.map((product, index) => (
              <ProductCard 
                ...
                product={product} 
                index={index}
                // الانتقال لصفحة تفاصيل الطقم عند النقر
          ...
          <div className="text-center py-24">
            <div className="mb-4 flex justify-center">
              <Sparkles size={40} className="text-[#fce4ec]" />
            </div>
            <p className="text-[#a68b90] font-medium">قريباً.. سنضيف أجمل تنسيقات التنانير!</p>
            <button 
              onClick={() => navigate('/')}
              className="mt-6 text-[#ff8da1] text-sm font-bold underline"
            >
              اكتشفي أقساماً أخرى
            </button>
          </div>
...
export default SkirtCombinations;