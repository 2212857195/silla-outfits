...
const RamadanDresses = () => {
  const navigate = useNavigate();

  // فلترة المنتجات لتشمل فقط "فساتين رمضان"
  const filteredProducts = productsData.filter(
    (product) => product.category === "ramadan-dresses"
  );

  // لضمان بدء الصفحة من الأعلى
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex flex-col items-center flex-1">
            <h1 className="text-xl font-bold text-[#8e5d67] flex items-center gap-2">
              <Moon size={18} className="text-[#ffb7c5] fill-[#ffb7c5]" />
              فساتين رمضان
            </h1>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-24">
            <div className="mb-4 flex justify-center">
              <Moon size={40} className="text-[#fce4ec] fill-[#fce4ec]" />
            </div>
            <p className="text-[#a68b90] font-medium">قريباً.. أرقى التنسيقات الرمضانية المحتشمة!</p>
            <button 
              onClick={() => navigate('/')}
              className="mt-6 text-[#ff8da1] text-sm font-bold underline"
            >
              اكتشفي المجموعات الأخرى
            </button>
          </div>
        )}
      </div>
