...
const PantCombinations = () => {
  const navigate = useNavigate();

  // فلترة المنتجات لتشمل فقط "تركيبات البنطلونات"
  const filteredProducts = productsData.filter(
    (product) => product.category === "pant-combinations"
  );
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8">
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-8">
            {filteredProducts.map((product, index) => (
              <ProductCard 
                ...
                product={product} 
                index={index}
                // عند الضغط يفتح صفحة تفاصيل القطع المكونة للطقم
          ...
          <div className="text-center py-24">
            ...
            <p className="text-[#a68b90] font-medium">نعمل حالياً على تجهيز أرقى تنسيقات البنطلونات!</p>
            <button 
              onClick={() => navigate('/')}
              className="mt-6 text-[#ff8da1] text-sm font-bold underline transition-colors hover:text-[#d81b60]"
            >
              اكتشفي الأقسام المتوفرة
            </button>
          </div>
        )}
      </div>
...
export default PantCombinations;