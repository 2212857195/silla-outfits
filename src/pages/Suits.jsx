...
const Suits = () => {
  ...
  // فلترة المنتجات لتشمل فقط "البدلات النسائية"
  const filteredProducts = productsData.filter(
    (product) => product.category === "suits"
  );
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8">
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-8">
            {filteredProducts.map((product, index) => (
              <ProductCard 
                ...
                product={product} 
                index={index}
                // الانتقال لصفحة تفاصيل القطع عند النقر
          ...
          <div className="text-center py-24">
            ...
            <p className="text-[#a68b90] font-medium">قريباً.. سنعرض لكِ أفخم البدلات النسائية بتنسيقات عصرية!</p>
            <button 
              onClick={() => navigate('/')}
